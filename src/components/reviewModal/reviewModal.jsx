...
const ReviewModal = ({modal, setModal, setReviews, reviews}) => {
    ...
    const addReview = (e) => {
        ...
        let today = new Date();
        let dateString = today.toDateString().slice(4, today.length).split(' ')
        let date = dateString[1] + " " + dateString[0] + " " + dateString[2]

        setReviews([...reviews, {id: reviews.length + 1, name: name, text: text, date: date, rate: rate}])
        setModal(false)
        setText('')
        setName('')
        setRate(2)
    ...
    return (
        <div className={modal ? cl.wrapper : classNames(cl.wrapper, cl.hide)} onMouseDown={() => setModal(false)}>
            <form className={cl.window} onMouseDown={(e) => e.stopPropagation()} onSubmit={(e) => addReview(e)}>
                ...
                <div className={cl.inputsArea}>
                    <MyInput placeholder={"Name"} value={name} onChange={(e) => setName(e.target.value)} required/>
                    ...
                    <TextareaAutosize value={text}
                                      ...
                                      onKeyDown={(e) => checkForEnter(e)} id={"text_area"} autoFocus required/>
                </div>
            </form>
        </div>
    );
